...
| Brand agents (`brand_agent/`, `labels/`) | Done: simvastatin + clarithromycin, verbatim passages auto-discovered from every numbered contraindications/interactions/dosing/indications/geriatric-use section (not hand-picked - `scripts/build_label.py`), topic tags inferred the same way, signed answers, Ed25519 identity. Labels self-refresh from openFDA when stale (`label_age_hours`/`refresh_if_stale`, `DEFAULT_MAX_AGE_HOURS`). Claude picks sections only if `ANTHROPIC_API_KEY` is set; otherwise keyword matching (`selectionMode` says which), which also matches patient context (age, renal/liver function, current medications). |
| Identity verification | **Partly live**: DNS key lookup + signed challenge are real; registry log + revocation are simulated. The DNSSEC problem is fixed. Mutual authentication: agents verify who is calling them (`SCRIPTSYNC_REQUIRE_CALLER_IDENTITY`). The remaining blocker is keys on the demo laptop (see "Keys"). |
| Web UI (`web/`) | Done: chat, Sources/Rules/Activity panels, presenter demo guide, skimmable answers (a "Key point" sentence on top of each passage, the passage drawn as lines with the label's own headings, question words highlighted; all display-only, see `assistant/readable.py`). The Sources panel also shows a linked agent's public GoDaddy ANS registration. Optional patient context (see `web/patient-context-template.txt`) is forwarded to agents, never logged or stored. |
...
```
- `assistant/`: `server.py` (API), `verify.py` (identity), `understand.py` (question analysis, PHI guard), `merge.py` (overlaps/gaps), `readable.py` (display-only reading aids: adds `view` offsets to each passage, never changes `text`), `log.py` (audit log, `logs/audit.jsonl`).
